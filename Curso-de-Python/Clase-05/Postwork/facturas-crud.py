...
def obtiene_id(tabla):
    """
    Imprime la lista de registros de tabla y solicita al usuario elegir un
    registro en base al id. Regresa el valor elegido por el usuario.
    """
    registros = obtiene_registros(tabla)
    print("\n" + tabla + "s disponibles "+"-"*30)
    imprime_texto(registros)
    print()
    # Se hace un ciclo infinito para leer una respuesta válida
    while True:
        # Se lee la respuesta del usuario
        resp = input("Seleccionar el número de registro de {}: ".format(tabla))
        # Se valida si es entero y si es una de las opciones disponibles
        try:
            resp = int(resp)
            # Se valida si resp está en la lista, si no se lanza una excepción
            [r[0] for r in registros[1:]].index(resp)
            # Si no hay excepción entonces el valor si existe y se regresa
            print()
            return resp
        except ValueError:
            print("\nEl valor indicando no es está en la lista, intente de nuevo\n")

@crud.command()
@click.argument("lugar")
@click.argument("rfcemisor")
def c_factura(lugar, rfcemisor):
    """
    Inserta un registro en la tabla Factura

    \b
    Argumentos:
    LUGAR     Es una cadena indicando el lugar de emisión de la factura
    RFCEMISOR Es el RFC de la organización que emite la factura
    """
    # Variables que definen que insertar y en que tabla se insertan
    tabla = "Factura"
    fecha = datetime.datetime.today()  # Fecha de hoy en type datetime
    fecha = fecha.isoformat()  # Fecha de hoy en tipo str en formato iso
    idcliente = obtiene_id("Cliente")
    idproducto = obtiene_id("Producto")

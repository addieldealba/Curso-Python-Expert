...
"""
Agregar al script lista-personas-alumnos.py la función guarda_alumnos()
que reciba una lista de alumnos o personas y que guarde la lista en el
archivo de texto llamado lista.txt
"""
...
def guardar_lista(lista, agregar=False):
    """
    Guarda la lista en el archivo lista.txt donde cada elemento debe contar
    una representación en str.
    """
    ...
    with open("lista.txt", modo) as flista:
        # Imprime separador
        flista.write("-" * maxcols + "\n")
        # Imprime cada registro
        for registro in lista:
            flista.write(str(registro) + "\n")
        # Imprime otro separador
        flista.write("-" * maxcols + "\n")
...
# Se hace uso de una función cuya única tarea es guardar la lista
guardar_lista(lista_personas)
guardar_lista(lista_alumnos, agregar=True)
